...
const run = async () => {
  try {
    ...
    // Update the mains course to 'away'
    let updated = false;
    order.courses = order.courses.map(course => {
      if (course.type === "Mains" && course.status === "on hold") {
        updated = true;
    ...
    if (updated) {
      ...
      await order.save();
      console.log("Mains course sent away successfully!");
    } else {
      console.log("No 'Mains' course in 'on hold' status to update.");
    }
